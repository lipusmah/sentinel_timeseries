...
if __name__ == "__main__":
    ...
    qq = f"SELECT * from raba_2018 WHERE RABA_ID = {RABA} AND Area(GEOMETRY) > {min_area}"
    cur = conn.execute(qq)
    logg = logger()
    iters = 20
    ...
    #     start = time.time()
    #     poly_id = next(cur)[0]
    #     mean_ndvis, std_ndvis, min_ndvis, max_ndvis, median_ndvis, dates = get_index_data_procedure(conn, poly_id, layer, logg)
